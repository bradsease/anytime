...
const SERIES_LENGTH: usize = 1_000;
...
fn benchmark_conversion<S, T>(
    ...
    TimeSeries<S>: Into<TimeSeries<T>>,
{
    group.bench_function(name, |bencher| {
        bencher.iter_batched(
            || TimeSeries::new(input.to_vec()),
            |series| {
                let converted: TimeSeries<T> = series.into();
                black_box(converted)
            },
...
macro_rules! benchmark_pair {
    ($group:expr, $source:ty, $target:ty) => {{
        let input = (0..SERIES_LENGTH)
            .map(|offset| Time::<$source>::from_jd(REFERENCE_JD + offset as f64 / 86_400.0))
            .collect::<Vec<_>>();
...
fn time_series_conversions(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("time_series_conversions");
    // This normalizes batch results for comparison with scale_conversions.
    group.throughput(Throughput::Elements(SERIES_LENGTH as u64));
...
criterion_group!(benches, time_series_conversions);
